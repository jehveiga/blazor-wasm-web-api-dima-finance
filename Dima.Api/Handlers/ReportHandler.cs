...
public class ReportHandler(AppDbContext context) : IReportHandler
{
        ...
        GetIncomesAndExpensesRequest request)
    {
        ...
        try
        {
            List<IncomesAndExpenses> data = await context
                .IncomesAndExpenses
                .AsNoTracking()
        ...
        GetIncomesByCategoryRequest request)
    {
        ...
        try
        {
            List<IncomesByCategory> data = await context
                .IncomesByCategories
                .AsNoTracking()
                .Where(x => x.UserId == request.UserId)
                .OrderByDescending(x => x.Year)
        ...
        GetExpensesByCategoryRequest request)
    {
        ...
        try
        {
            List<ExpensesByCategory> data = await context
    ...
    public async Task<Response<FinancialSummary?>> GetFinancialSummaryReportAsync(GetFinancialSummaryRequest request)
    {
        ...
        DateTime startDate = new(DateTime.Now.Year, DateTime.Now.Month, 1, 0, 0, 0, DateTimeKind.Local);
        try
        {
            FinancialSummary? data = await context
                .Transactions
                .AsNoTracking()
                .Where(
