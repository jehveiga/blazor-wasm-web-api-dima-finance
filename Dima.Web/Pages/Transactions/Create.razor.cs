...
public partial class CreateTransactionPage : ComponentBase
{
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        try
        {
            ...
            PagedResponse<List<Category>?> result = await CategoryHandler.GetAllAsync(request);
            if (result.IsSuccess)
            {
    ...
    public async Task OnValidSubmitAsync()
    {
        ...
        try
        {
            Response<Transaction?> result = await TransactionHandler.CreateAsync(InputModel);
            if (result.IsSuccess)
            {
